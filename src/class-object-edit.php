...
class Object_Edit {
	...
	public function render_page() : void {
		...
		$this->save_form();
		?>
		<h2><?php _e( 'GMOP Edit Object', GMOP_TD ); ?>&nbsp;<a href="<?php echo 'admin.php?page=gmop-objects'; ?>" class="button add-new-h2"><?php _e( 'Show Objects', GMOP_TD ); ?></a></h2>
		...
		$gmopobject = $wpdb->get_row( "SELECT * FROM $table_name WHERE ID = '$theid'", OBJECT );
		if ( ! empty( $gmopobject ) ) {
			$this->print_scripts( $gmopobject );
			$this->display_form( $gmopobject );
		} else {
			echo '<div id="message" class="error fade"><p>' . __( 'Object not found!', GMOP_TD ) . '</p></div>';
		}
	...
	protected function save_form() : void {
		...
		$marker      = ! empty( $_POST['gmop_marker'] ) ? absint( $_POST['gmop_marker'] ) : 0;

		if ( ! empty( $theid ) && ! empty( $title ) && ! empty( $latitude ) && ! empty( $longitude ) && ! empty( $marker ) ) {
			// TODO: Prepare for SQL injection
			$query = "UPDATE $table_name SET title = '$title', description = '$description', url = '$url', latitude = '$latitude', longitude = '$longitude', marker = '$marker' WHERE ID = '$theid'";
			$results = $wpdb->query( $query );
			$message = __( 'Object updated.', GMOP_TD );
		} else {
			$message = __( 'Object was not updated.', GMOP_TD );
		}
	 ...
	 * @return object
	 */
	protected function get_object( int $theid ) : object {
		global $wpdb;

		$table_name = $wpdb->gmop_objects;
		$object     = $wpdb->get_row( "SELECT * FROM $table_name WHERE ID = '$theid'", OBJECT );
	 ...
	 * Display form.
	 *
	 * @param object $gmopobject Object.
	 *
	 * @return void
	 */
	protected function display_form( object $gmopobject ) : void {
	?>
		...
		<p class="submit"><input type="submit" name="submit" class="button button-primary" value="<?php _e( 'Update Object', GMOP_TD ); ?>" /> </p>
		</form>
	 ...
	 * @return void
	 */
	protected function print_scripts( object $gmopobject ) : void {
